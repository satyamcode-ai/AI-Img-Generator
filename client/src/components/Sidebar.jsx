...
const Sidebar = ({isMenuOpen,setIsMenuOpen}) => {
  ...
  return (
    <div className={`flex flex-col h-screen min-w-72 p-5 dark:bg-gradient-to-b from-[#242124]/30 to-[#000000]/50  border-r border-[#80609F]/50 backdrop-blur-xl transition-all duration-500 max-md:absolute left-0 z-1 ${!isMenuOpen && 'max-md:-translate-x-full'}`}>
      ...
      {/* new chat button */}
      <button className="flex items-center justify-center w-full mt-10 py-2.5 bg-gradient-to-r from-[#3D81F6] to-[#A456F7] rounded-md font-medium text-sm transition-all duration-300 cursor-pointer text-white hover:from-[#2A6CEB] hover:to-[#8945E5]">
        <span className="mr-2 test-xl"><svg xmlns="http://www.w3.org/2000/svg" 
     fill="none" 
     viewBox="0 0 24 24" 
     strokeWidth={2} 
     stroke="currentColor" 
  ...
  <path strokeLinecap="round" strokeLinejoin="round" d="M12 4v16m8-8H4" />
      ...
      <p className="text-xs text-center mt-2">Purchase credits to use QuickGPT</p>
...
<div className='flex items-center gap-3 p-3 mt-4 border border-gray-300 dark:border-white/15 rounded-md cursor-pointer group'>
  ...
  <img onClick={()=> setIsMenuOpen(false)} src={assets.close_icon} className="absolute top-3 right-3 w-5 h-5 cirsor-pointer md:hidden not-dark:invert" alt="" />
    </div>
  );
};
